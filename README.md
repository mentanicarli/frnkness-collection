...
пробелов и знаков по краям, поэтому запятую в конце можно не копировать. Если
строка встречается в песне несколько раз (например, в припеве), разбор
показывается у каждого повтора.
